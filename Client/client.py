...
class GUI:
    ...
    def control(self, client):
        ...
        window_width = 600
        window_height = 300
        # Set the window dimensions using global variables
        ...
        self.process = tk.Button(self.top, text="Process Running", height=14, bg='#EEE0C9', fg='black', command=lambda: self.processrunning(client), **button_params)
        self.process.grid(row=1, column=0, rowspan=5)

        self.app = tk.Button(self.top, text="App Running", width=40, height=3, bg='#EEE0C9', fg='black', command=lambda: self.apprunning(client), **button_params)
        self.app.grid(row=1, column=1, columnspan=3)

        self.shut = tk.Button(self.top, text="Shutdown", width=18, height=5, bg='#EEE0C9', fg='black', command=lambda: self.shutdown(client), **button_params)
        self.shut.grid(row=2, column=1, columnspan=1, rowspan=2, sticky="nsew")

        self.capture = tk.Button(self.top, text="Screenshot", width=18, height=5, bg='#EEE0C9', fg='black', command=lambda: self.takepicture(client), **button_params)
        self.capture.grid(row=2, column=2, columnspan=1, rowspan=2, sticky="nsew")

        self.key = tk.Button(self.top, text="Keystroke", height=12, width=11, bg='#EEE0C9', fg='black', command=lambda: self.keystroke(client), **button_params)
